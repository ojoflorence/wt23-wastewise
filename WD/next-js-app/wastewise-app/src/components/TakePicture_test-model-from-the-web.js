...
const TakePicture = () => {
    ...
    const JSONURL = 'https://storage.googleapis.com/tfjs-models/savedmodel/mobilenet_v2_1.0_224/model.json';
    ...
    const classifyImage = async (dataURL) => {
        // const model = await loadGraphModel(`data:application/json,${JSON.stringify(modelJson)}`);
        const model = await loadGraphModel(JSONURL);
            console.log('LOG model.inputs[0].shape :');
            console.log(model.inputs[0].shape);
            console.log('LOG model.outputs[0].shape :');
            console.log(model.outputs[0].shape);
            console.log('LOG model :');
            console.log(model);

        const browserimage = new Image();
        browserimage.src = dataURL;
        await browserimage.decode();
        // const image = tf.browser.fromPixels(browserimage);
        // console.log(image.shape);

        const inputTensor = tf.browser.fromPixels(browserimage)
            .toFloat()
            .div(255)
            .expandDims() // add the extra dimension at the beginning
            .resizeNearestNeighbor([224, 224]); // resize the image to the expected size
        const prediction = model.execute({ images: inputTensor });
            console.log('LOG prediction :');
    ...
    if (dataURL !== null) {
        const result = classifyImage(dataURL);
        console.log(result);
    }
